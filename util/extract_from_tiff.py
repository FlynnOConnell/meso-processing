...
def mroi_from_tiff(
    ...
    Args:
        metadata_json (dict): JSON representation of the metadata.
        metadata_kv (dict): Key-value pairs of specific metadata attributes.
        data (numpy.ndarray): The 4D numpy array containing image data with dimensions corresponding
            to [height, width, channel, volume].
        num_channels (int): Number of channels in the image data.
        num_volumes (int): Number of volumes (or timepoints) in the image data.
    ...
    Example:
        ...
        >>> rois = mroi_from_tiff(metadata_keyvals, metadata_kv, image_data, 2, 10, 5)
    """
